...
**E-mail de réinitialisation (Brevo, optionnel)**  
Sans **`BREVO_API_KEY`** et **`BREVO_SENDER_EMAIL`**, le backend en production **n’envoie pas** de mail (avertissement dans les logs). Pour activer l’envoi : (1) compte Brevo et expéditeur vérifié ; (2) authentification DNS du domaine (DKIM/DMARC) pour une bonne délivrabilité — voir `documentation/ENVIRONMENT_SETUP.md` ; (3) dans GitHub → **Secrets** : `BREVO_API_KEY` ; → **Variables** : `BREVO_SENDER_EMAIL`, `BREVO_SENDER_NAME` ; (4) redéployer pour que le workflow et le compose injectent les variables dans le conteneur backend. Les liens « mot de passe oublié » utilisent **`CORS_ORIGIN`** / **`CORS_ORIGINS`** comme origine de confiance (le client peut proposer une base d’URL, mais seules les origines autorisées sont acceptées — pas d’open redirect).
...
**Version affichée (en bas à gauche)** : l’API `/api/version` (souvent le **backend**, Nginx proxifiant `/api`) renvoie **`RELEASE_LABEL`** : le numéro du produit dans le `package.json` **racine** du dépôt, suivi optionnellement de `+` et des 7 premiers caractères du commit (ex. `1.3.0+a1b2c3d`). Le workflow de déploiement définit `RELEASE_LABEL` ; `GIT_SHA` reste disponible pour le build. Si vous voyez « vdev », le label n’a pas été passé au build ou une ancienne image tourne encore.
