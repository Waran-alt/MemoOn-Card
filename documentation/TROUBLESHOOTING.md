...
| pgAdmin: "Name does not resolve" for host `postgres` | pgAdmin container on a different Docker network | `docker network ls`; inspect postgres container network | Run pgAdmin with `--network db-admin-net`; use Host **postgres**, Port **5432**. See `documentation/PGADMIN.md`. |
...
| Backend logs: `command not found: docker` or compose script says docker CLI missing | Root `yarn dev` runs `docker compose` on the **host**. That was started inside the backend container (e.g. Docker Desktop **Command** `yarn dev` with **Working directory** `/app`, or an override that drops the compose `command`) | Inspect container command / working dir in Docker Desktop | Use the default compose backend service (see `docker-compose.yml`), or set working dir to `/app` and command to `yarn workspace @memoon-card/backend dev`. On the host, keep using `yarn dev` or `yarn docker:up` to start the stack. |
| Frontend: `can't open 'scripts/docker-dev-entrypoint.sh'` | **Working directory** is not `/app/frontend` (e.g. `/app`), so the relative path from compose does not exist | Inspect frontend container working dir in Docker Desktop | Restore default compose settings, or use command `/bin/sh /app/frontend/scripts/docker-dev-entrypoint.sh` (already the default in `docker-compose.yml` after recent fixes). Ensure `./frontend` is bind-mounted to `/app/frontend`. |
