...
describe('AppPage', () => {
  ...
  it('shows loading then empty state when no decks', async () => {
    ...
    expect(mockGet).toHaveBeenCalledWith('/api/decks', expect.objectContaining({ signal: expect.any(AbortSignal) }));
  });
  ...
  it('calls POST /api/decks and adds deck to list on create submit', async () => {
    ...
    mockGet.mockResolvedValue({ data: { success: true, data: [newDeck] } });
