...
export default function FlaggedCardsPage() {
  ...
  return (
    <div className="mc-study-page mx-auto max-w-3xl space-y-6">
      <div>
        <h2 className="text-lg font-semibold text-[var(--mc-text-primary)]">{ta('flaggedCardsTitle')}</h2>
        <p className="mt-1 text-sm text-[var(--mc-text-secondary)]">{ta('flaggedCardsIntro')}</p>
      </div>
      {resolveError && (
        <p className="text-sm text-[var(--mc-accent-danger)]" role="alert">{resolveError}</p>
      )}
      {loading && <p className="text-sm text-[var(--mc-text-secondary)]">{tc('loading')}</p>}
      {error && (
        <p className="text-sm text-[var(--mc-accent-danger)]" role="alert">{error}</p>
      )}
      {!loading && !error && rows.length === 0 && (
        <p className="text-sm text-[var(--mc-text-secondary)]">{ta('flaggedCardsEmpty')}</p>
      )}
      {!loading && !error && rows.length > 0 && (
        <ul className="space-y-3">
          {rows.map((flag) => (
            <li key={flag.id} className="rounded-lg border border-[var(--mc-border-subtle)] bg-[var(--mc-bg-surface)] p-4">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1 text-xs text-[var(--mc-text-secondary)]">
                    {ta('flaggedCardsDeck')}:{' '}
                    <Link href={`/${locale}/app/decks/${flag.deck_id}`} className="text-[var(--mc-accent-primary)] underline hover:no-underline">
                      {flag.deck_title}
                    </Link>
                  </p>
      ...
      <p className="text-xs text-[var(--mc-text-secondary)]">
        <Link href={`/${locale}/app`} className="text-[var(--mc-accent-primary)] underline hover:no-underline">
          {ta('backToDecks')}
        </Link>
      </p>
    </div>
