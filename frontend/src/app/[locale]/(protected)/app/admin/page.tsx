...
export default function AdminPage() {
  ...
  const { data, loading, error, refetch } = useApiGet<{ flags: FeatureFlagRow[] }>('/api/admin/feature-flags', {
    errorFallback: ta('adminLoadError'),
  });
  ...
  async function updateFlag(flag: FeatureFlagRow, patch: Partial<Pick<FeatureFlagRow, 'enabled' | 'rolloutPercentage'>>) {
    ...
    setMessage(null);
    try {
      await apiClient.patch(`/api/admin/feature-flags/${flag.flagKey}`, {
  ...
  async function loadOverrides(flagKey: string) {
    ...
    try {
      const res = await apiClient.get<{ success: boolean; data?: { rows?: FeatureFlagOverrideRow[] } }>(
        `/api/admin/feature-flags/${flagKey}/overrides?limit=50`
      );
  ...
  async function upsertOverride() {
    ...
    setMessage(null);
    try {
      await apiClient.put(`/api/admin/feature-flags/${selectedFlagKey}/overrides/${overrideUserId.trim()}`, {
  ...
  async function removeOverride(userId: string) {
    ...
    setMessage(null);
    try {
      await apiClient.delete(`/api/admin/feature-flags/${selectedFlagKey}/overrides/${userId}`);
  ...
  return (
    <div className="mc-study-page mx-auto max-w-5xl space-y-6">
      <div>
        <h2 className="text-lg font-semibold text-[var(--mc-text-primary)]">{ta('adminTitle')}</h2>
        <p className="mt-1 text-sm text-[var(--mc-text-secondary)]">{ta('adminIntro')}</p>
      </div>
