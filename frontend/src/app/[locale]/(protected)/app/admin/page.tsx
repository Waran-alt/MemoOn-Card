...
export default function AdminPage() {
    ...
    return (
      <div className="mc-study-page mx-auto max-w-2xl space-y-4">
        <div className="rounded-xl border border-[var(--mc-border-subtle)] bg-[var(--mc-bg-surface)] p-6 text-center shadow-sm">
          <h2 className="text-lg font-semibold text-[var(--mc-text-primary)]">{ta('adminAccessDeniedTitle')}</h2>
  ...
  return (
    <div className="mc-study-page mx-auto max-w-2xl space-y-6">
      ...
      <div className="rounded-xl border border-[var(--mc-border-subtle)] bg-[var(--mc-bg-surface)] p-6 text-center shadow-sm">
        <p className="text-sm text-[var(--mc-text-secondary)]">{ta('adminUserManagementComingSoon')}</p>
      </div>
    </div>
  );
}
